...
namespace nam
{
    ...
    void TransformComponent::GetLocalYPR(float& yaw, float& pitch, float& roll) const
    {
        YPRFromQuaternion(m_localRotation, yaw, pitch, roll);
    }
    ...
    void TransformComponent::GetWorldYPR(float& yaw, float& pitch, float& roll) const
    {
        YPRFromQuaternion(m_worldRotation, yaw, pitch, roll);
    }

    void TransformComponent::MoveLocalForward(float distance)
    {
        UpdateDirectionVectors();
        XMFLOAT3 move = {
            m_forward.x * distance,
            m_forward.y * distance,
            m_forward.z * distance
        };
        TranslateLocal(move);
    }

    void TransformComponent::MoveWorldForward(float distance)
    {
        XMFLOAT3 worldForward = GetWorldForward();
        XMFLOAT3 move = {
            worldForward.x * distance,
            worldForward.y * distance,
            worldForward.z * distance
        };
        TranslateWorld(move);
    }
    ...
    XMFLOAT3 TransformComponent::GetWorldForward()
    {
        UpdateWorldData();
        return GetForward();
    }
    ...
    void TransformComponent::UpdateWorldForward(float yaw, float pitch, float roll)
    {
        GetWorldForward();
    }
